...
//     It uses data for the WT to estimate parameters for the wild-type
...
transformed parameters {
  ...
  real log_mean_y0;
  vector[N_lig_wt] log_mean_y1;
  vector[N_lig_wt] log_mean_y2;
...
model {
  ...
  log_y0_wt ~ normal(log_mean_y0, sigma*log_y0err_wt);
  log_y1_wt ~ normal(log_mean_y1, sigma*log_y1err_wt);
  log_y2_wt ~ normal(log_mean_y2, sigma*log_y2err_wt);
...
generated quantities {
  ...
  vector[19] fc1_out;
  vector[19] fc2_out;
  vector[N_lig_wt] dev_log_y1;       // log_mean_y - log_y
  vector[N_lig_wt] dev_log_y2;       // log_mean_y - log_y
  
  dev_log_y1 = log_mean_y1 - log_y1_wt;
  dev_log_y2 = log_mean_y2 - log_y2_wt;
  ...
  for (i in 1:19) {
    ...
    c3 = R/N_NS * exp(-delta_eps_RA_wt);
    c1 = (1 + x_out[i]/K_A_1_wt)^hill_n;
    c2 = ( (1 + x_out[i]/K_I_1_wt)^hill_n ) * exp(-delta_eps_AI_wt);
	
    y1_out[i] = g_max/(1 + (c1/(c1+c2))*c3);
    fc1_out[i] = 1/(1 + (c1/(c1+c2))*c3);
	
    c1 = (1 + x_out[i]/K_A_2_wt)^hill_n;
    c2 = ( (1 + x_out[i]/K_I_2_wt)^hill_n ) * exp(-delta_eps_AI_wt);
